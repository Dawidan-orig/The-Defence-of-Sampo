...
public class SwordFighter_StateMachine : MeleeFighter
{
    ...
    [Tooltip("Всё, что имеет скорость выше этого значения - блокируется")]
    ...
    [Tooltip("Указывает значения от 0 до 1 означающие вероятность выбора удара слева направо поверху.")]
    ...
    protected override void FixedUpdate()
    {
        ...
        //TODO :
        // Пока что было решено не разбегаться в стороны, так, типа, выглядит лучше.
        // Мне нужны референсы.
        /*
        #region Отходим от союзников
        Vector3 retireDirection = Vector3.zero;
        int amount = 0;

        foreach (GameObject go in AttackCatcher.Controlled)
        {
            if (go == null)
                continue;

            if (go.TryGetComponent(out SwordFighter_StateMachine _)
                && go.TryGetComponent(out Faction faction) && !faction.IsWillingToAttack(GetComponent<Faction>().f_type))
            {
                amount++;
                retireDirection += (go.transform.position - transform.position).normalized;
            }
        }
        retireDirection = -retireDirection / amount;

        if (amount > 0)
            GetComponent<MovingAgent>().MoveIteration(transform.position + retireDirection, transform.position);
        #endregion
        */
    ...
    {
        //TODO : Тут надо всё перепроверить и за-Refactor'ить
