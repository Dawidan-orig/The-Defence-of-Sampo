...
namespace WingedCore.Core.Utility.Deep
{
    //Git-amend : "The Unity HACK that the PROS know"
    public static class PlayerLoopUtils
    {
        ...
        public static bool InsertSystem<T>(ref PlayerLoopSystem loop, in PlayerLoopSystem target, int index) 
        {
            if(loop.type != typeof(T)) return HandleSubSystemLoop<T>(ref loop, target, index);

            var playerLoopSystemList = new List<PlayerLoopSystem>();
            if(loop.subSystemList != null) playerLoopSystemList.AddRange(loop.subSystemList);
            playerLoopSystemList.Insert(index, target);
            loop.subSystemList = playerLoopSystemList.ToArray();
            return true;
        }
